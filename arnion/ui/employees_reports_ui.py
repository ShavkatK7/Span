...
class EmployeesReportWindow:
    ...
    def get_report_text(self):
        ...
        report_text += "______________________________________________________" + os.linesep
        data_rows = EmployeeDataHandler.select_list_rpt()
        current_department_name = "c46f4954-43ae-477c-adf2-1fd2da5cfd96"
        for data_row in data_rows:
            # Если новые отдел, добавляем заголовок группы
            if data_row.department_name != current_department_name:
                report_text += data_row.department_name + os.linesep
                current_department_name = data_rows.department_name

            #  Добавляем запись
            report_text += "         " + data_rows.get_full_name() + os.linesep
        return report_text
