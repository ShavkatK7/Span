...
class EmployeesWindow:
    ...
    def __init__(self):
        ...
        self.lbl_title = tk.Label(self.window, text="Сотрудники", font=('Hevletica', 16, 'bold'), fg='#0000cc',
                                  justify='center')
    ...
    def add_record_callback(self, added_data_row: EmployeeDataObject):
        ...
        self.lbox_data_rows.curselection_set('end')
    ...
    def edit_record_callback(self, edited_data_row: EmployeeDataObject):
        ...
        self.data_row[self.selection] = edited_data_row
    ...
    def delete_record(self):
        ...
        if not answer:
            return
        self.selection = self.lbox_data_rows.curselection()[0]
        id = self.data_rows[self.selection].emloyee_id
...
class EmployeeWindow:
    ...
    def __init__(self, add_new: bool, data_row: EmployeeDataObject,
        ...
        # Добавление метки заголовка
        self.lbl_title = tk.Label(self.window, text=title_text, font=('Hevletica', 16, 'bold'), fg='#0000cc',
                                  justify='center')
        self.lbl_title.place(x=25, y=15, width=450, height=50)

        # Добавление полей ввода
        lbl_first_name = tk.Label(self.window, text="Имя: ", font=('Hevletica', 10, 'bold'))
        lbl_first_name.place(x=20, y=85)

        self.ent_first_name = tk.Entry(self.window, font=('Hevletica', 10, 'bold'))
        self.ent_first_name.place(x=115, y=85, width=370, height=25)
        self.ent_first_name.insert(tk.END, data_row.first_name)

        # Добавление полей ввода
        lbl_middle_name = tk.Label(self.window, text="Отчество: ", font=('Hevletica', 10, 'bold'))
        lbl_middle_name.place(x=20, y=120)

        self.ent_middle_name = tk.Entry(self.window, font=('Hevletica', 10, 'bold'))
        self.ent_middle_name.place(x=115, y=120, width=370, height=25)
        self.ent_middle_name.insert(tk.END, data_row.middle_name)

        # Добавление полей ввода
        lbl_last_name = tk.Label(self.window, text="Фамилия: ", font=('Hevletica', 10, 'bold'))
        lbl_last_name.place(x=20, y=155)

        self.ent_last_name = tk.Entry(self.window, font=('Hevletica', 10, 'bold'))
        self.ent_last_name.place(x=115, y=155, width=370, height=25)
        self.ent_last_name.insert(tk.END, data_row.last_name)

        # Добавление полей ввода
        lbl_department = tk.Label(self.window, text="Отдел: ", font=('Hevletica', 10, 'bold'))
        lbl_department.place(x=20, y=190)

        self.cbo_department = ttk.Combobox(self.window, font=('Hevletica', 10, 'bold'))
        self.init_combobox()
        ...
        self.btn_ок = tk.Button(self.window, text="Сохранить",
                                 font=('Hevletica', 10, 'bold'), bg='#ccffcc', command=self.save)
        self.btn_ок.place(x=140, y=240, width=90, height=30)

        # Добавление кнопки "Отмена"
        self.btn_cancel = tk.Button(self.window, text="Отмена",
                                font=('Hevletica', 10, 'bold'), bg='#ffeeee', command=self.close)
        self.btn_cancel.place(x=250, y=240, width=90, height=30)

    # Фунция заполнения выпадающего списка
    ...
    # Функция закрытия этого окна без сохранения
    def close(self):
        self.window.destroy()

    # Функция сбора информации с полей ввода
    def collect_from_control(self):
